...
(function () {
    ...
    function I(b, a, c) {
        ...
        a = d[0];
        c = d[1];
        b.fact && (d = b.fact[0], b = b.fact[1], d = F(a * d, c * b), a = d[0], c = d[1]);
        64 < c && (d =
            F(Math.round(64 * a / c) || 1, 64), q("denominator too small: %d/%d rounded to %d/%d", [a, c, d[0], d[1]]), a = d[0], c = d[1]);
        ...
        for (var c in this.vnums) {
            if (a.lline ||
                ...
                if (d) {
                    ...
                    3 == this.nvlt ? a.ixp + parseInt(c) == Math.min.apply(null, C(this.vnums)) && (f += a.lnum) : 4 == this.nvlt ? parseInt(c) == Math.min.apply(null, C(this.vnums)) && (f += a.lnum) : a.lnum && (f += a.lnum, this.repbra = 1);
                    d.str = f
            ...
            for (var d =
                this.lyrics[c], f = {}, e = d.reduce(function (a, b) {
                return a.concat(C(b))
    ...
    r.prototype.outVoices = function (b, a) {
        ...
        g = {};
        k = Math.min.apply(null, C(this.vnums));
        for (h in this.vnums)if (0 != this.cnt.getv("note", h)) {
            ...
            p.add("V:" + this.vceCnt);
            this.repbra && (1 == this.nvlt && 1 < this.vceCnt && p.add("I:repbra 0"), 2 == this.nvlt && parseInt(h) > k && p.add("I:repbra 0"));
    ...
    D.prototype.info = function (b, a) {
        this.infolist.push(("undefined" == typeof a || a ? "-- " : "") + b)
    };
    D.prototype.mkHeader = function (b, a,
                                     c) {
        var d = [], f = [], e, g, l, k, h, m;
        k = b.slice();
        ...
        for (h in this.clefs) {
            ...
            for (m = 0; m < k.length; ++m)if (e =
                    k[m].nt, l = k[m].step, a = k[m].midi, (g = k[m].nhd) || (g = "normal"), J(e) != a || e != l)0 < this.volpan && d.push("%%MIDI drummap " + e + " " + a + "\n"), d.push("I:percmap " + e + " " + l + " " + a + " " + g + "\n");
    ...
    n.prototype.matchSlur = function (b, a, c, d, f, e) {
        if (-1 != ["start", "stop"].indexOf(b))if (a || (a = "1"), a in this.slurBuf) {
            ...
            b != l ? (c != k || "start" != l || g && e || (h.before = "(" + h.before, d.after += ")"), delete this.slurBuf[a]) :
                (q("double slur numbers %s-%s in part %d, measure %d, voice %d note %s, first discarded", [b, a, this.msr.ixp + 1, this.msr.ixm + 1, c, d.ns]), this.slurBuf[a] = [b, c, d, f])
    ...
    n.prototype.doNote = function (b) {
        ...
        var d = 0 < b.find("chord").length,
            f = b.find("pitch>step").text() || b.find("unpitched>display-step").text(), e = b.find("pitch>octave").text() || b.find("unpitched>display-octave").text(), g = 0 < b.find("rest").length, l = b.find("time-modification>actual-notes").text();
        if (l) {
        ...
        a.before = "";
        a.after = "";
        a.grace && !this.ingrace &&
        (this.ingrace = 1, a.before = "{", "yes" == l.attr("slash") && (a.before += "/"));
        ...
        if (g || "no" != b.attr("print-object")) {
            ...
            if (b.find("unpitched").length) {
                var k = this.curClef[this.curStf[c]], f = Q(f, parseInt(e), k, this.tstep), e = b.find("instrument"), e = e.length ? e.attr("id") : "dummyId",
                    e = this.drumInst[e] || J(g), k = b.find("notehead"), h = k.text().replace(" ", "-");
                "yes" == k.attr("filled") && (h += "+");
                "x" == h && (g = "^" + g.replace(/\^/g, "").replace(/_/g, ""));
                if ("circle-x" == h || "diamond" == h)g = "_" + g.replace(/\^/g, "").replace(/_/g, "");
    ...
    n.prototype.doAttr = function (b) {
        ...
        d && (e = parseInt(d), g = b.find("key>mode").first().text() || "major", h = "FCGDAEB".split(""), k = "Cb Gb Db Ab Eb Bb F C G D A E B F# C#".split(" "), l = "Ab Eb Bb F C G D A E B F# C# G# D# A#".split(" "),
            d = "", "major" == g && (d = k[7 + e]), "minor" == g && (d = l[7 + e] + "min"), e = 0 <= e ? y(h.slice(0, e), x(e, 1)) : y(h.slice(e), x(-e, -1)), e = [d, e], d = e[0], this.msralts = e[1], f && !c && "none" == p.key ? p.key = d : d == p.key && f || (this.msr.attr += "[K:" + d + "]"));
        ...
        g = b.find("clef");
        for (e = 0; e < g.length; e++)if (l = $(g[e]),
                d = parseInt(l.attr("number") || "1"), k = l.find("sign").text(), h = "percussion" != k ? l.find("line").text() || "" : "", h = a[k + h] || "", k = l.find("clef-octave-change").text() || "0", h += {
                    "-2": "-15",
                    "-1": "-8",
                    1: "+8",
                    2: "+15"
                }[k] || "", this.clefOct[d] = -parseInt(k), c && (h += " transpose=" + c), (k = b.find("staff-details>staff-lines").text()) && (h += " stafflines=" + k), this.curClef[d] = h, f)this.clefMap[d] = h; else for (l = this.stfMap[d], n = 0; n < l.length; ++n)m = l[n], d != this.curStf[m] && (t = d - this.curStf[m], this.curStf[m] = d, k = 0 < t ? "+" : "", this.msc.appendElem(m,
            "[I:staff " + k + t + "]")), this.msc.appendElem(m, "[K:" + h + "]")
    };
    n.prototype.doDirection = function (b) {
        ...
        c = b.find("sound");
        if (c.length) {
            if (g = c.find("midi-instrument")) {
                l = c.find("midi-instrument>midi-program").text();
                k = c.find("midi-instrument>midi-channel").text();
                for (h in this.vceInst)this.vceInst[h] == g.attr("id") && (d = h);
                (h = (l ? l - 1 : k) + "") && 0 < p.volpan && this.msc.appendElem(d, "[I:MIDI= " + (l ? "program" :
                        "channel") + " " + h + "]")
            }
            if (c = c.attr("tempo"))c = -1 < c.indexOf(".") ? parseFloat(c).toFixed(2) : parseInt(c), 0 == this.msc.tijd && 0 == this.msr.ixm ? p.tempo = c : this.msc.appendElem(d, "[Q:1/4=" + c + "]")
        }
        b = b.children("direction-type");
        if (b.length) {
            c = b.find("wedge");
            if (c.length) {
                switch (c.attr("type")) {
                ...
                this.msc.appendElem(d, f)
            }
            f =
                b.find("words").eq(0);
            f.length && (a = "below" == a ? "_" : "^", 0 > parseFloat(f.attr("default-y") || "0") && (a = "_"), (f = f.text().replace(/"/g, '\\"').replace(/\n/g, " ").trim()) && this.msc.appendElem(d, '"' + a + f + '"'));
            for (e in O)a = O[e], b.find("dynamics>" + e).length && this.msc.appendElem(d, a);
            b.find("coda").length && this.msc.appendElem(d, "O");
            b.find("segno").length && this.msc.appendElem(d, "S")
        }
    };
    n.prototype.doHarmony = function (b) {
        ...
        c = {
            major: "",
            minor: "m", augmented: "+", diminished: "dim", dominant: "7", "half-diminished": "m7b5"
        };
        ...
        k = "";
        h = b.find("kind").text();
        h in c ? h = c[h] : -1 < h.indexOf("-") ? (c = h.split("-"), h = c[0], c = c[1], h = (d[h] || "") + (f[c] || ""), 0 == h.indexOf("sus") && (k = h, h = "")) : "none" ==
        h && (h = b.find("kind").attr("text"));
        d = b.find("degree");
        for (f = 0; f < d.length; ++f)c = $(d[f]), h += (e[c.find("degree-alter").text()] || "") + c.find("degree-value").text();
        h = h.replace("79", "9").replace("713", "13").replace("maj6", "6");
    ...
    n.prototype.doBarline = function (b) {
        ...
        "right" == b.attr("location") &&
        (a = b.find("bar-style").text(), "light-light" == a ? this.msr.rline = "||" : "light-heavy" == a && (this.msr.rline = "|]"));
    ...
    n.prototype.doPartList = function (b) {
        ...
        for (a = 0; a < f.length; ++a) {
            ...
            for (c = 0; c < g.length; ++c) {
                ...
                (k = l.find("midi-unpitched").text()) && (this.drumInst[l.attr("id")] =
                    k - 1)
            }
            this.instMid.push(e)
        }
    ...
    n.prototype.mkTitle = function (b) {
        var a, c, d = [], f = [], e = [], g, l, k, h, m;
        a = b.find("work>work-title").text().trim();
        c = b.find("movement-title").text().trim();
        ...
        g = b.find("identification>rights");
        for (l = 0; l <
        g.length; ++l)h = $(g[l]).text(), h = h.split("\n").map(function (a) {
            return a.trim()
        }), f.push.apply(f, h);
        g = b.find("credit");
        ...
        e = function (b) {
            ...
            var k = [], h, l;
            for (l = 0; l < e.length; ++l)h = e[l], 6 > b && (h && -1 < a.indexOf(h) || h && -1 < c.indexOf(h)) || 5 > b && (h && -1 < d.indexOf(h) || h && -1 < f.indexOf(h)) || 4 > b && (a && -1 < h.indexOf(a) || c && -1 < h.indexOf(c)) || 3 > b && (d.some(g) ||
            f.some(g)) || 2 > b && /^[\d\W]*$/.test(h) || k.push(h);
            0 == b && a + c && (k = "");
            return k
        }(this.ctf);
        a && (a = "T:" + a.replace(/\n/g, "\nT:") + "\n");
        ...
        a && (p.title = a.substr(0, a.length - 1));
    ...
    n.prototype.parse = function (b) {
        ...
        for (var c = 0; c < b.length; ++c) {
            ...
            d = this.msc.outVoices(this.msr.divs,
                c);
            this.addStaffMap(d);
            this.addMidiMap(c, d)
        }
