...
const abc2svg = {version: "1.4.9", vdate: "2016-01-18"}
function Abc(user) {
    var wpsobj, svgobj;
    this.user = user;
    ...
    SL_ABOVE = 1, SL_BELOW = 2, SL_AUTO = 3, SL_HIDDEN = 4, SL_DOTTED = 8, OPEN_BRACE = 1, CLOSE_BRACE = 2, OPEN_BRACKET = 4, CLOSE_BRACKET = 8, OPEN_PARENTH = 16, CLOSE_PARENTH = 32, STOP_BAR = 64, FL_VOICE = 128, OPEN_BRACE2 = 256, CLOSE_BRACE2 = 512, OPEN_BRACKET2 = 1024, CLOSE_BRACKET2 = 2048, MASTER_VOICE = 4096, BASE_LEN = 1536, CM = 37.8, IN = 96, YSTEP = 128;
    var glovar = {
        clef: {type: CLEF, clef_auto: true, clef_type: "a"},
        meter: {type: METER, wmeasure: 1, a_meter: []}
    }, info = {}, parse = {ctx: {}, prefix: "%", state: 0};
    ...
    function error(sev, s, msg) {
        if (s && s.ctx)errbld(sev, msg, s.ctx.fname, s.istart); else errbld(sev, msg)
    }
    ...
    function scanBuf() {
        ...
        this.get_int = function () {
            var val = 0, c = this.buffer[this.index];
            while (c >= "0" && c <= "9") {
                val = val * 10 + Number(c);
                c = this.next_char()
            }
    ...
    function y_get(st, up, x, w) {
        var y, p_staff = staff_tb[st], i = Math.floor(x / realwidth * YSTEP), j = Math.floor((x + w) / realwidth * YSTEP);
        if (i < 0)i = 0;
        if (j >= YSTEP) {
            j = YSTEP - 1;
            if (i > j)i = j
    ...
    function y_set(st, up, x, w, y) {
        var p_staff = staff_tb[st], i = Math.floor(x / realwidth * YSTEP), j = Math.floor((x + w) / realwidth * YSTEP);
        if (i < 0)i = 0;
        if (j >= YSTEP) {
            j = YSTEP - 1;
            if (i > j)i = j
    ...
    function d_near(de) {
        ...
        if (up)s.ymx = y + dd.h; else s.ymn = y;
        de.y = y;
    ...
    function d_pf(de) {
        ...
        if (!up)de.y -= dd.h;
        if (dd.str)set_str(de, dd.str); else if (dd.name != "sfz")de.str = dd.name
    }
    ...
    function d_trill(de) {
        ...
        if (up)y += dd.h;
        y_set(st, up, x, w, y);
        if (up)s.ymx = s2.ymx = y; else s.ymn = s2.ymn = y
    }
    ...
    function d_upstaff(de) {
        ...
        switch (dd.glyph) {
            ...
            case"sphr":
                yc = stafft + 1;
                for (s = s.ts_next; s; s = s.ts_next)if (s.shrink)break;
                if (s)x += (s.x - x) * .4; else x += (realwidth - x) * .4;
                break;
            default:
    ...
    function deco_build(text) {
        ...
        var c_func = Number(a[2]), h = parseFloat(a[4]), wl = parseFloat(a[5]), wr = parseFloat(a[6]);
        if (isNaN(c_func)) {
            error(1, null, "%%deco: bad C function value '" + a[2] + "'");
            return undefined
        }
        if ((c_func < 0 || c_func >= func_tb.length) && (c_func < 32 || c_func > 41)) {
        ...
        if (c == "(" || c == ")" && dd.name.indexOf("(") < 0) {
            if (c == "(")dd.ldst = true; else dd.lden = true;
            name2 = dd.name.slice(0, -1) + (c == "(" ? ")" : "(");
            dd2 = dd_tb[name2];
            if (dd2) {
                if (c == "(")dd.ld_en_dd = dd2; else dd2.ld_en_dd = dd
            }
        }
        return dd
    }
    ...
    function deco_cnv(a_dcn, s, prev) {
        ...
        for (i = 0; i < n; i++) {
            ...
            if (!dd) {
                dd = deco_def(dcn);
                if (!dd) {
                    if (cfmt.decoerr)error(1, s, "Unknown decoration '" + dcn + "'");
                    continue
                }
            }
            switch (dd.func) {
                ...
                case 36:
                    ...
                    if (dd.name[6] == "1")s.beam_br1 = true; else s.beam_br2 = true;
                    break;
                case 37:
                    s.rbstop = 1;
                    break;
                case 38:
                    ...
                    if (s.nflags > 0)s.nflags += s.ntrem; else s.nflags = s.ntrem;
                    break;
                case 39:
                    if (s.type != NOTE) {
                        error(1, s, "!" + dd.name + "! must be on a note");
    ...
    function draw_all_deco() {
        ...
        while (1) {
            ...
            i = f.indexOf("/");
            if (i > 0) {
                if (de.s.stem >= 0)f = f.slice(0, i); else f = f.slice(i + 1)
            }
            st = de.st;
            y = de.y + staff_tb[st].y;
            if (dd.func >= 6 && !cfmt.dynalign && (de.flags.up && st > 0 || !de.flags.up && st < nstaff)) {
                if (de.flags.up)ym = ymid[--st]; else ym = ymid[st++];
                ym -= dd.h * .5;
            ...
            if (de.flags.val) {
                if (dd.func != 2 || stv_g.st < 0)out_deco_val(x, y, f, de.val / stv_g.scale, de.defl); else out_deco_val(x, y, f, de.val, de.defl)
            } else if (de.str) {
    ...
    function create_deco(s) {
        var dd, k, l, pos, de, n = s.a_dd.length;
        for (k = 0; k < n; k++) {
            dd = s.a_dd[k];
            switch (dd.func) {
                default:
            ...
            de = {s: s, dd: dd, st: s.st, ix: a_de.length, flags: {}, defl: {}, dy: 0};
            a_de.push(de);
    ...
    function draw_deco_note() {
        ...
        for (i = 0; i < n_de; i++) {
            ...
            dd2 = dd.ld_en_dd;
            if (!dd2) {
                dd2 = {name: dd.name.slice(0, -1) + ")", func: dd.func, glyph: dd.glyph, h: dd.h};
                dd.ld_en_dd = dd2
            }
            v = de.s.v;
            for (j = i + 1; j < n_de; j++) {
    ...
    function draw_deco_staff() {
        var s, first_gchord, p_voice, x, y, w, i, n, v, de, dd, gch, gch2, ix, minmax = new Array(nstaff);
        ...
        function draw_repbra(p_voice) {
            ...
            for (; s; s = s.next) {
                ...
                x = s1.x;
                if (s.type != BAR) {
                    if (s.rbstop)w = 0; else w = s.x - realwidth + 4
                    ...
                    if (s1.st > 0 && !(cur_sy.staves[s1.st - 1].flags & STOP_BAR)) {
                        ...
                        w = 12
                    } else if (s.bar_type[0] != ":" || s.bar_type == "]") {
                        w = 0
                    } else {
                        w = 8
                    }
                ...
                if (!s.next && !s.rbstop && !p_voice.bar_start) {
                    ...
                    p_voice.bar_start.text = undefined;
                    p_voice.bar_start.rbstart = 1;
                    delete p_voice.bar_start.a_gch
                }
                if (s1.text)xy_str(x + 4, y2 - gene.curfont.size * .9, s1.text);
                xypath(x, y2);
                if (s1.rbstart == 2)output.push("m0 20v-20");
                output.push("h");
                output.push(w.toFixed(2));
                if (s.rbstop == 2)output.push("v20");
                output.push('"/>\n');
        ...
        for (i = 0; i <= nstaff; i++)minmax[i] = {ymin: 0, ymax: 24};
        for (s = tsfirst; s; s = s.ts_next) {
            if (!s.a_gch)continue;
        ...
        for (i = 0; i <= nstaff; i++)minmax[i] = {ymin: 0, ymax: 0};
        n = a_de.length;
        for (i = 0; i < n; i++) {
            de = a_de[i];
            dd = de.dd;
        ...
        for (i = 0; i < n; i++) {
            ...
            if (cfmt.dynalign) {
                if (de.flags.up)y = minmax[de.st].ymax; else y = minmax[de.st].ymin;
                de.y = y
            } else {
                y = de.y
            }
    ...
    function draw_partempo(st, top) {
        ...
        if (some_tempo) {
            ...
            for (s = tsfirst; s; s = s.ts_next) {
                if (!s.seqst)continue;
                g = s.extra;
                for (; g; g = g.next)if (g.type == TEMPO && !g.deleted)break;
                if (!g)continue;
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            g = s.extra;
            if (!g)continue;
            for (; g; g = g.next)if (g.type == PART)break;
            if (!g)continue;
            if (!some_part) {
                some_part = true;
                set_font("parts");
    ...
    function calculate_beam(bm, s1) {
        ...
        if (!s1.beam_st) {
            s = sym_dup(s1);
            s.prev = s1.prev;
            if (s.prev)s.prev.next = s; else voice_tb[s.v].sym = s;
            s1.prev = s;
            s.next = s1;
        ...
        for (s = s1; ; s = s.next) {
            ...
            if (s.stem >= 0)x = stem_xoff + s.notes[0].shhd; else x = -stem_xoff + s.notes[s.nhd].shhd;
            x *= scale;
            x += s.x;
            s.xs = x;
        ...
        if (!s1.grace) {
            ...
            if (a >= 0)a = BEAM_SLOPE * a / (BEAM_SLOPE + a); else a = BEAM_SLOPE * a / (BEAM_SLOPE - a)
        } else {
            if (a > BEAM_SLOPE)a = BEAM_SLOPE; else if (a < -BEAM_SLOPE)a = -BEAM_SLOPE
        }
        a0 = a * (s2.xs - s1.xs) / (20 * (notes - 1));
        if (a0 * a0 < BEAM_THRESH * BEAM_THRESH)a = 0;
        b = (sy - a * sx) / notes;
        if (cfmt.flatbeams) {
            if (!s1.grace)b = -11 + staff_tb[st].y; else b = 35 + staff_tb[st].y;
            a = 0
        }
        max_stem_err = 0;
        s = s1;
        if (two_dir) {
            if (!s1.grace)ys = BEAM_SHIFT; else ys = 3;
            ys *= nflags - 1;
            ys += BEAM_DEPTH;
            ys *= .5;
            if (s1.stem != s2.stem && s1.nflags < s2.nflags)ys *= s2.stem; else ys *= s1.stem;
            b += ys
        } else if (!s1.grace) {
            ...
            for (; s && s.time <= s2.time; s = s.ts_next) {
                ...
                if (s.v == v) {
                    if (s.nhd == 0)stem_err = min_tb[0][s.nflags]; else stem_err = min_tb[1][s.nflags];
                    if (s.stem > 0) {
                    ...
                    if (s1.stem > 0) {
                        if (s.stem > 0) {
                            ...
                            if (s.v > v)stem_err = s.ymx - ys; else stem_err = s.ymn + 8 - ys
                        } else {
                            stem_err = s.ymx - ys
                        }
                    } else {
                        if (s.stem < 0) {
                            ...
                            if (s.v < v)stem_err = ys - s.ymn; else stem_err = ys - s.ymx + 8
                        } else {
                            stem_err = ys - s.ymn
                        }
                    }
            ...
            for (; ; s = s.next) {
                ...
                stem_err = GSTEM - 2;
                if (s.stem > 0)stem_err -= ys - 3 * (s.notes[s.nhd].pit - 18); else stem_err += ys - 3 * (s.notes[0].pit - 18);
        ...
        if (!two_staves && !two_dir)for (s = s1.next; ; s = s.next) {
            var g;
            switch (s.type) {
                case REST:
                    g = s.ts_next;
        ...
        for (s = s1; ; s = s.next) {
            ...
            switch (s.type) {
                ...
                case REST:
                    ...
                    if (s.head != "full")y = Math.floor((y + 3 + 12) / 6) * 6 - 12;
                    s.y = y;
                    break
            }
            if (s == s2)break
    ...
    function draw_beams(bm) {
                ...
                if (k1 == k2) {
                    if (k1 == s1) {
                        ...
                        if (k.beam_br1 || k.beam_br2 && i > 2) {
                            ...
                            if (k1.nflags < k.nflags || k1.nflags == k.nflags && k1.dots < k.dots)x1 += bstub; else x1 -= bstub
                        }
                    }
                }
        ...
        if (s1.tmp)unlksym(s1); else if (s2.tmp)unlksym(s2)
    }
    ...
    function draw_lstaff(x) {
        ...
        function draw_sysbra(x, st, flag) {
            ...
            if (flag & (CLOSE_BRACE | CLOSE_BRACE2))out_brace(x, yb, yt - yb); else out_bracket(x, yt, yt - yb)
        }
    ...
    function draw_meter(x, s) {
        ...
        for (i = 0; i < s.a_meter.length; i++) {
            ...
            if (meter.top == "C|")dx = 13; else dx = 13 * meter.top.length;
            if (meter.bot) {
    ...
    function draw_keysig(p_voice, x, s) {
        ...
        if (!s.k_a_acc) {
            ...
            if (s.k_sf < 0) {
                ...
                if (cfmt.cancelkey && s.k_sf > old_sf) {
                    x += 2;
                    for (; i > old_sf; i--) {
                        xygl(x, staffb + shift, "acc3");
                        shift += p_seq[-i];
            ...
            for (i = 0; i < s.k_a_acc.length; i++) {
                ...
                if (i != 0 && (shift > last_shift + 18 || shift < last_shift - 18))x -= 5.5; else if (acc.acc != last_acc)x += 3;
                last_acc = acc.acc;
    ...
    function draw_bar(s, bot, h) {
        ...
        if (st != 0 && s.ts_prev && (s.ts_prev.type != BAR || s.ts_prev.st != st - 1))h = staff_tb[st].topbar * staff_tb[st].staffscale;
        var dotted = s.bar_dotted || s.bar_type == ":", bar_type = bar_cnv(s.bar_type);
        if (!bar_type)return;
        for (i = bar_type.length; --i >= 0;) {
            switch (bar_type[i]) {
                case"|":
                    set_sscale(-1);
                    if (dotted)out_dotbar(x, bot, h); else out_bar(x, bot, h);
                    break;
                default:
                    x -= 3;
                    set_sscale(-1);
    ...
    function draw_basic_note(x, s, m, y_tb) {
        ...
        if (no_head) {
            ...
            i = p.indexOf("/");
            if (i >= 0) {
                if (s.stem >= 0)p = p.slice(0, i); else p = p.slice(i + 1)
            }
        } else if (s.grace) {
            p = "ghd";
    ...
    function draw_note(s, fl) {
        ...
        if (!s.invisible && !s.stemless) {
            ...
            if (!fl || nflags <= 0) {
                if (s.nflags > 0) {
                    if (s.stem >= 0)slen -= 1; else slen += 1
                }
        ...
        if (!s.invisible && fl && s.trem1) {
            var ntrem = s.ntrem ? s.ntrem : 0, x1 = x;
            slen = 3 * (s.notes[s.stem > 0 ? s.nhd : 0].pit - 18);
            if (s.head == "full" || s.head == "empty") {
                x1 += (s.grace ? GSTEM_XOFF : 3.5) * s.stem;
                if (s.stem > 0)slen += 6 + 5.4 * ntrem; else slen -= 6 + 5.4
            } else {
                if (s.stem > 0)slen += 5 + 5.4 * ntrem; else slen -= 5 + 5.4
            }
            slen /= voice_tb[s.v].scale;
    ...
    function prev_scut(s) {
        ...
        if (s.next.type == KEY)s = s.next;
        if (s.next.type == METER)return s.next;
        return s
    }
    ...
    function slur_out(x1, y1, x2, y2, dir, height, dotted) {
        ...
        if (!dotted)output.push('<path class="fill" d="M'); else output.push('<path class="stroke" stroke-dasharray="5,5" d="M');
        out_sxsy(x1, " ", y1);
    ...
    function draw_slur(k1_o, k2, m1, m2, slur_type) {
        var k1 = k1_o, k, g, x1, y1, x2, y2, height, addy, a, y, z, h, dx, dy, dir;
        ...
        if (k1_o != k2) {
            ...
            for (k = k2.ts_next; k; k = k.ts_next)if (k.new_sy)break;
            if (!k)x2 = realwidth; else x2 = k.x
        }
        if (m1 >= 0) {
        ...
        if (m2 >= 0) {
            ...
            if (k2.type == NOTE) {
                if (dir > 0) {
                    if (k2.stem > 0) {
                        x2 += 1;
                        if (k2.beam_st && k2.nflags >= -1 && !k2.in_tuplet)y2 = k2.ys - 6; else y2 = k2.ys + 3
                    } else {
                        y2 = k2.y + 8
                    }
                } else {
        ...
        if (k2.type != NOTE) {
            if (k1.type == NOTE)y2 = y1 + 1.2 * dir; else y2 = y1;
        ...
        if (a > SLUR_SLOPE || a < -SLUR_SLOPE) {
            ...
            if (a * dir > 0)y1 = y2 - a * (x2 - x1); else y2 = y1 + a * (x2 - x1)
        }
        y = y2 - y1;
        ...
        if (nn > 3)height = (.08 * (x2 - x1) + 12) * dir; else height = (.03 * (x2 - x1) + 8) * dir;
        if (dir > 0) {
        ...
        if (k1.v == k2.v)for (k = k1; k != k2; k = k.next) {
            if (k.st != upstaff)continue;
            y = a * k.x + addy;
            if (k.ymx < y)k.ymx = y; else if (k.ymn > y)k.ymn = y;
            if (k.next == k2) {
                dx = x2;
                if (k2.sl1)dx -= 5
            } else {
    ...
    function draw_slurs(first, last) {
        ...
        while (1) {
            ...
            if (s.slur_start) {
                ...
                for (m1 = 0; m1 <= s.nhd; m1++)if (s.notes[m1].sl1)break;
            ...
            if ((k.type == NOTE || k.type == REST || k.type == SPACE) && (k.slur_end || k.sl2)) {
                if (k.slur_end) {
                    ...
                    for (m2 = 0; m2 <= k.nhd; m2++)if (k.notes[m2].sl2)break;
                    k.notes[m2].sl2--;
                    k.sl2--
                }
            } else {
    ...
    function draw_tuplet(t, s) {
        ...
        if (nb_only) {
            ...
            xm = (s2.x + s1.x) * .5;
            if (s1 == s2)a = 0; else a = (s2.ys - s1.ys) / (s2.x - s1.x);
            b = s1.ys - a * s1.x;
            yy = a * xm + b;
            if (s1.stem > 0) {
            ...
            if (s1.stem * s2.stem > 0) {
                if (s1.stem > 0)xm += 1.5; else xm -= 1.5
            }
            ym = a * xm + b;
            if (t.tuplet_f[2] == 0)out_bnum(xm, ym, t.tuplet_p); else out_bnum(xm, ym, t.tuplet_p + ":" + t.tuplet_q);
            if (s1.stem > 0) {
                ym += 10;
        ...
        if (s1.multi >= 0) {
            ...
            if (s1.st == upstaff) {
                s3 = s1;
                if (s3.type != NOTE) {
                    for (s3 = s3.next; s3 != s2; s3 = s3.next)if (s3.type == NOTE)break
                }
            ...
            if (s2.st == upstaff) {
                s3 = s2;
                if (s3.type != NOTE) {
                    for (s3 = s3.prev; s3 != s1; s3 = s3.prev)if (s3.type == NOTE)break
                }
                ym = y_get(upstaff, 1, s3.x, 0);
                if (ym > y2)y2 = ym
            }
            if (s2.dur > s2.prev.dur) {
                if (s2.next)x2 = s2.next.x - s2.next.wl - 5; else x2 = realwidth - 6
            } else {
                x2 = s2.x + 4;
            ...
            x1 = s1.x - 7;
            if (s2.dur > s2.prev.dur) {
                if (s2.next)x2 = s2.next.x - s2.next.wl - 8; else x2 = realwidth - 6
            } else {
                x2 = s2.x + 2;
        ...
        yy = .5 * (y1 + y2);
        if (t.tuplet_f[2] == 0)out_bnum(xm, yy, t.tuplet_p, true); else out_bnum(xm, yy, t.tuplet_p + ":" + t.tuplet_q, true);
        if (s1.multi >= 0) {
            yy += 9;
    ...
    function draw_note_ties(k1, k2, mhead1, mhead2, job) {
        ...
        for (i = 0; i < mhead1.length; i++) {
            ...
            sh = k1.notes[m1].shhd;
            if (dir > 0) {
                if (m1 < k1.nhd && p1 + 1 == k1.notes[m1 + 1].pit)if (k1.notes[m1 + 1].shhd > sh)sh = k1.notes[m1 + 1].shhd
            } else {
                if (m1 > 0 && p1 == k1.notes[m1 - 1].pit + 1)if (k1.notes[m1 - 1].shhd > sh)sh = k1.notes[m1 - 1].shhd
            }
            x1 += sh * .6;
            x2 = k2.x;
            sh = k2.notes[m2].shhd;
            if (dir > 0) {
                if (m2 < k2.nhd && p2 + 1 == k2.notes[m2 + 1].pit)if (k2.notes[m2 + 1].shhd < sh)sh = k2.notes[m2 + 1].shhd
            } else {
                if (m2 > 0 && p2 == k2.notes[m2 - 1].pit + 1)if (k2.notes[m2 - 1].shhd < sh)sh = k2.notes[m2 - 1].shhd
            }
            x2 += sh * .6;
            st = k1.st;
            switch (job) {
                case 0:
                    if (p1 == p2 || p1 & 1)p = p1; else p = p2;
                    break;
                case 1:
                case 3:
                    x1 = k1.x;
    ...
    function draw_all_ties(p_voice) {
        ...
        for (s2 = s1; s2; s2 = s2.next) {
            ...
            if (s2.text[0] == "1")s_rtie = p_voice.s_tie; else p_voice.s_tie = s_rtie
        }
        if (!s2)return;
        if (p_voice.s_tie) {
        ...
        while (1) {
            for (s1 = s2; s1; s1 = s1.next) {
                ...
                for (s2 = s1.next; s2; s2 = s2.next)if (s2.type == NOTE)break;
                if (!s2) {
                    s1 = null;
                    break
                }
            ...
            if (!s2) {
                for (s2 = s1.ts_next; s2; s2 = s2.ts_next) {
                    if (s2.st != s1.st)continue;
                    if (s2.time < time)continue;
                    if (s2.time > time) {
                        s2 = null;
                        break
                    }
                ...
                if (s2.time != time)s2 = tie_comb(s1)
            }
    ...
    function draw_sym_near() {
        ...
        for (v = 0; v < voice_tb.length; v++) {
            var bm = {}, first_note = true;
            p_voice = voice_tb[v];
            for (s = p_voice.sym; s; s = s.next) {
                for (g = s.extra; g; g = g.next) {
                    if (g.type != NOTE)continue;
                    if (g.beam_st && !g.beam_end)calculate_beam(bm, g)
                }
                if (s.type != NOTE)continue;
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            ...
            if (s.notes[0].acc) {
                y = s.y;
                if (s.notes[0].acc == 1 || s.notes[0].acc == 3)y -= 7; else y -= 5;
                if (s.ymn > y)s.ymn = y;
    ...
    function draw_vname(indent) {
        ...
        for (v = 0; v < voice_tb.length; v++) {
            ...
            if (!staff_d[st])staff_d[st] = p; else staff_d[st] += "\\n" + p
        }
    ...
    function set_staff() {
        ...
        for (st++; st <= nstaff; st++) {
            ...
            if (sy_staff_prev.sep)staffsep = sy_staff_prev.sep; else staffsep = cfmt.sysstaffsep;
            if (sy_staff_prev.maxsep)maxsep = sy_staff_prev.maxsep; else maxsep = cfmt.maxsysstaffsep;
            dy = 0;
    ...
    function bar_set(bar_bot, bar_height) {
        ...
        for (st = 0; st <= cur_sy.nstaff; st++) {
            ...
            bar_height[st] = dy - staff_tb[st].y - bot;
    ...
    function draw_systems(indent) {
        var next_sy, s, s2, st, x, x2, res, xstaff = [], bar_bot = [], bar_height = [];
                ...
                for (st = 0; st <= nstaff; st++) {
                    ...
                    if (x < 0) {
                        if (next_sy.staves[st] && !next_sy.staves[st].empty) {
                            if (s.type == BAR)xstaff[st] = s.x; else xstaff[st] = s.x - s.wl - 2
                        }
                        continue
                    }
                    if (s.ts_prev.type == BAR)x2 = s.ts_prev.x; else x2 = s.x - s.wl - 2;
                    if (!next_sy.staves[st] || next_sy.staves[st].empty)xstaff[st] = -1; else if (next_sy.staves[st].stafflines == cur_sy.staves[st].stafflines)continue; else xstaff[st] = x2;
                    draw_staff(st, x, x2)
                }
                cur_sy = next_sy;
                res = bar_set();
        ...
        for (s = p_voice.sym; s; s = s.next) {
            ...
            switch (s.type) {
                ...
                case CLEF:
                    ...
                    if (user.anno_start)anno_start(s);
                    y = staff_tb[st].y;
                    if (s.clef_name)xygl(x, y + s.y, s.clef_name); else if (!s.clef_small)xygl(x, y + s.y, s.clef_type + "clef"); else xygl(x, y + s.y, "s" + s.clef_type + "clef");
                    if (s.clef_octave) {
                        if (s.clef_octave > 0) {
                            y += s.ymx - 10;
                            if (s.clef_small)y -= 1
    ...
    function set_tie_room() {
        ...
        for (v = 0; v < voice_tb.length; v++) {
            ...
            for (; s; s = s.next) {
                if (!s.ti1)continue;
                if (s.notes[0].pit < 20 && (s.notes[0].ti1 & 7) == SL_BELOW); else if (s.notes[s.nhd].pit > 24 && (s.notes[s.nhd].ti1 & 7) == SL_ABOVE); else continue;
                s2 = s.next;
                ...
                if (dx < 100)dy = 9; else if (dx < 300)dy = 12; else dy = 16;
    ...
    var cfmt = {
        ...
        partsspace: 8,
        pagewidth: 21 * CM,
        pos: {dyn: 0, gch: 0, gst: 0, orn: 0, ste: 0, voc: 0, vol: 0},
        rightmargin: .7 * IN,
        rbmax: 4,
        rbmin: 2,
        scale: 1,
    ...
    function param_set_font(xxxfont, param) {
        ...
        if (!font) {
            ...
            if (!scale)scale = 1.1;
            font = {name: new_name, size: Number(new_size), swfac: new_size * scale};
            font_tb[new_fn] = font
        }
        cfmt[xxxfont] = new_fn
    }
    ...
    function set_infoname(param) {
        ...
        for (var i = 0; i < tmp.length; i++) {
            ...
            if (param.length == 1)tmp.splice(i, 1); else tmp[i] = param;
    ...
    const textopt = {align: "j", center: "c", fill: "f", justify: "j", ragged: "f", right: "r", skip: "s"};
    ...
    function set_pos(k, a) {
        var v, val, pos;
        if (a[0] >= "0" && a[0] <= "3")val = parseInt(a); else val = posval[a] || 0;
        switch (k[0]) {
            case"d":
                k = "dyn";
                break;
            case"g":
                if (k[1] == "c")k = "gch"; else k = "gst";
                break;
            case"o":
                k = "orn";
                break;
        ...
        pos[k] = val;
        if (!curvoice) {
            for (v = 0; v < voice_tb.length; v++)voice_tb[v].pos[k] = val
        }
    }
    ...
    function set_format(cmd, param, lock) {
        ...
        switch (cmd) {
            ...
            case"stretchlast":
                ...
                if (cmd == "scale")f /= .75; else if (cmd == "pagescale")cmd = "scale";
                cfmt[cmd] = f;
                break;
            case"bstemdown":
            case"breakoneoln":
            ...
            case"staffwidth":
                ...
                if (v < 0)parse.line.error("'staffwidth' too big"); else cfmt.rightmargin = v;
                break;
            case"textoption":
            ...
            case"voicecombine":
                ...
                if (curvoice && cmd == "combinevoices") {
                    for (f = 0; f < voice_tb.length; f++)voice_tb[f].combine = v
                }
    ...
    function style_add_font(font) {
        var name = font.name, i = name.indexOf("Italic"), j = 100, o = name.indexOf("Oblique"), b = name.indexOf("Bold");
    ...
    function get_font(xxx) {
        xxx += "font";
        var fn = cfmt[xxx], font = font_tb[fn];
        if (!font) {
        ...
        for (bol = 0; ; bol = eol + 1) {
            ...
            if (pscom) {
                ...
                if (b) {
                    ...
                    do_begin_end(b[1], a[1], parse.file.slice(eol + 1, i).replace(new RegExp("^" + line0 + line1, "gm"), ""));
                    eol = file.indexOf("\n", i + 6);
                    if (eol < 0)eol = eof;
                    continue
                }
                switch (a[0]) {
                    ...
                    case"voice":
                        ...
                        if (!select) {
                            if (parse.cur_tune_opts)parse.cur_tune_opts.voice_opts = null; else parse.voice_opts = null;
                            continue
                        }
                        ...
                        while (1) {
                            ...
                            bol += 2;
                            if (eol < 0)text = file.slice(bol); else text = file.slice(bol, eol);
                            a = text.split(/\s+/, 1);
                            switch (a[0]) {
                                default:
            ...
            switch (line0) {
                ...
                case"T":
                    ...
                    curvoice = voice_tb[0];
                    if (!curvoice) {
                        if (info.T == undefined)info.T = text; else info.T += "\n" + text;
                        continue
                    }
                    s = {type: BLOCK, subtype: "title", text: text};
                    sym_link(s);
                    continue;
                case"K":
                    if (parse.state == 0)continue;
                    if (parse.state == 1)info.K = text; else if (parse.state == 2)goto_tune();
                ...
                case"W":
                    ...
                    if (!info.W)info.W = text; else info.W += txt_add + text;
                    break;
                case"s":
                ...
                default:
                    if ("ABCDFGHOSZ".indexOf(line0) >= 0) {
                        ...
                        if (!info[line0])info[line0] = text; else info[line0] += txt_add + text;
                        break
                    }
                    parse.line.buffer = text;
                    parse.line.index = 0;
    ...
    const dx_tb = {full: 9, empty: 10, oval: 12, square: 15.3};
    ...
    function set_acc_shft() {
        ...
        function acc_shift(notes, dx_head) {
            ...
            for (i = n; --i >= 0;) {
                ...
                if (!dx) {
                    dx = notes[i].shhd;
                    if (dx < 0)dx = dx_head - dx; else dx = dx_head
                }
                ps = notes[i].pit;
                for (i1 = n; --i1 > i;) {
                    if (!notes[i1].acc)continue;
    ...
    function unlksym(s) {
        ...
        if (s.prev)s.prev.next = s.next; else voice_tb[s.v].sym = s.next;
        if (s.ts_next) {
        ...
        for (v = 0; v < voice_tb.length; v++) {
            ...
            for (s = p_voice.sym; s; s = s.next) {
                ...
                for (s1 = s.ts_prev; s1; s1 = s1.ts_prev) {
                    ...
                    if (s1.type == NOTE)if (s1.notes[0].pit < up)up = s1.notes[0].pit
                }
                if (up == 127) {
                    if (staff_chg)s.st++;
                    continue
                ...
                for (s1 = s.ts_next; s1; s1 = s1.ts_next) {
                    ...
                    if (s1.type == NOTE)if (s1.notes[s1.nhd].pit > down)down = s1.notes[s1.nhd].pit
                }
                if (down == -127) {
                    if (staff_chg)s.st++;
                    continue
    ...
    function set_graceoffs(s) {
        ...
        if (next && next.type == NOTE) {
            if (g.y >= 3 * (next.notes[next.nhd].pit - 18))xx -= 1; else if (g.beam_st && g.y < 3 * (next.notes[0].pit - 18) - 7)xx += 2
        }
        return xx
    }
    ...
    function set_width(s) {
        ...
        switch (s.type) {
            ...
            case REST:
                ...
                if (s2 && s2.type == GRACE)s.wl = wlnote - 4.5; else s.wl = wlw;
                return;
            case SPACE:
                xx = s.width / 2;
                s.wr = xx;
            ...
            case BAR:
                ...
                if (!s.invisible) {
                    ...
                    s.wl = w;
                    if (s.next && s.next.type != METER)s.wr = 8; else s.wr = 5
                } else {
                    s.wl = s.wr = 0
                }
            ...
            case KEY:
                ...
                if (!s.k_a_acc) {
                    n1 = s.k_sf;
                    if (s.k_old_sf && (cfmt.cancelkey || n1 == 0))n2 = s.k_old_sf; else n2 = 0;
                    if (n1 * n2 >= 0) {
                        if (n1 < 0)n1 = -n1;
                        if (n2 < 0)n2 = -n2;
                        if (n2 > n1)n1 = n2
                    ...
                    for (i = 1; i < n2; i++) {
                        acc = s.k_a_acc[i];
                        if (acc.pit > s.k_a_acc[i - 1].pit + 6 || acc.pit < s.k_a_acc[i - 1].pit - 6)n1--; else if (acc.acc != last_acc)esp += 3;
                        last_acc = acc.acc
                    }
                }
                s.wr = 5.5 * n1 + esp;
            ...
            case METER:
                ...
                for (i = 0; i < s.a_meter.length; i++) {
                    ...
                    if (meter.top == "C|") {
                        ...
                        if (!meter.bot || meter.top.length > meter.bot.length)w += 6.5 * meter.top.length; else w += 6.5 * meter.bot.length
                    }
                }
                s.wl = w;
                s.wr = w + 7;
    ...
    function set_space(s) {
        ...
        if (smallest_duration >= BASE_LEN / 2) {
            if (smallest_duration >= BASE_LEN)len /= 4; else len /= 2
        }
        if (len >= BASE_LEN / 4) {
            if (len < BASE_LEN / 2)i = 5; else if (len < BASE_LEN)i = 6; else if (len < BASE_LEN * 2)i = 7; else if (len < BASE_LEN * 4)i = 8; else i = 9
        } else {
            if (len >= BASE_LEN / 8)i = 4; else if (len >= BASE_LEN / 16)i = 3; else if (len >= BASE_LEN / 32)i = 2; else if (len >= BASE_LEN / 64)i = 1; else i = 0
        }
        ...
        if (!s.dur) {
            switch (s.type) {
                case BAR:
                    if (s.bar_type.length > 1)space *= .8; else space *= .7;
                    break;
                case CLEF:
                    space -= s.wl;
                    break
    ...
    function custos_add(s) {
        ...
        for (i = 0; i < s.notes.length; i++) {
            new_s.notes[i] = {pit: s2.notes[i].pit, shhd: 0, dur: BASE_LEN / 4}
        }
        new_s.stemless = true
    }
    ...
    function set_nl(s) {
        ...
        for (; ; s = s.ts_next) {
            ...
            switch (s.type) {
                case BAR:
                    if (done || !s.bar_num && s.next && s.bar_type[s.bar_type.length - 1] == ":" && s.bar_type[0] != ":")cut_here = true; else done = 1;
                    break;
                case STBRK:
                    if (!s.stbrk_forced) {
                        unlksym(s);
            ...
            if (cut_here)break;
            if (s.extra) {
                if (!extra)extra = s; else error(2, s, "Extra symbol may be misplaced")
            }
        }
        if (extra && extra != s) {
            s2 = extra.extra;
    ...
    function get_ck_width() {
        var v, p_voice, nv = voice_tb.length;
        for (v = 0; v < nv; v++) {
            p_voice = voice_tb[v];
        ...
        if (!cfmt.singleline)return p_voice.clef.wl + p_voice.clef.wr + p_voice.key.wl + p_voice.key.wr;
        set_width(p_voice.meter);
        return p_voice.clef.wl + p_voice.clef.wr + p_voice.key.wl + p_voice.key.wr + p_voice.meter.wl + p_voice.meter.wr
    }

    function get_width(first, last, w) {
        var s, shrink, space, sp_fac = 1 - cfmt.maxshrink;
        for (s = first; s != last; s = s.ts_next) {
            if (!s.seqst)continue;
            s.x = w;
            shrink = s.shrink;
            if ((space = s.space) < shrink)w += shrink; else w += shrink * cfmt.maxshrink + space * sp_fac
        }
        return w
    }
    ...
    function set_lines(first, last, lwidth, indent) {
        ...
        wwidth = get_width(first, last, indent);
        s = first;
        while (1) {
            ...
            if (!cut_here) {
                var beam = s2.dur && !s2.beam_st && !s2.beam_end ? 1 : 0, bar_time = s2.time;
                s = s2;
                s2 = s3 = null;
                xmax -= 6;
                for (; s != last; s = s.ts_next) {
    ...
    function set_yval(s) {
        switch (s.type) {
            case CLEF:
                ...
                if (s.ymn > -1)s.ymn = -1;
                if (s.clef_octave) {
                    if (s.clef_octave > 0)s.ymx += 12; else s.ymn -= 12
                }
                break;
            case KEY:
                if (s.k_sf > 2)s.ymx = 24 + 10; else if (s.k_sf > 0)s.ymx = 24 + 6; else s.ymx = 24 + 2;
                s.ymn = -2;
                break;
            default:
                s.ymx = 24 + 2;
    ...
    function set_auto_clef(st, s_start, clef_type_start) {
        ...
        for (s = s_start; s; s = s.ts_next) {
            ...
            if (s.notes[0].pit < min)min = s.notes[0].pit; else if (s.notes[s.nhd].pit > max)max = s.notes[s.nhd].pit
        }
        if (min >= 19 || min >= 13 && clef_type_start != "b")return "t";
        if (max <= 13 || max <= 19 && clef_type_start != "t")return "b";
        if (clef_type_start == "a") {
            if ((max + min) / 2 >= 16)clef_type_start = "t"; else clef_type_start = "b"
        }
    ...
    const delta_tb = {t: 0 - 2 * 2, c: 6 - 3 * 2, b: 12 - 4 * 2, p: 0 - 3 * 2};
    const rest_sp = [[18, 18], [12, 18], [12, 12], [8, 12], [6, 8], [10, 10], [6, 4], [10, 0], [10, 4], [10, 10]];
    ...
    function set_stem_dir() {
        ...
        while (s) {
            ...
            for (; s != u; s = s.ts_next) {
                ...
                st_v = st_v_tb[st];
                if (v_st && v_st.st2 >= 0) {
                    if (st == v_st.st1)s.multi = -1; else if (st == v_st.st2)s.multi = 1;
                    continue
                }
                if (st_v.length <= 1) {
                    if (s.floating) {
                        if (st == voice_tb[v].st)s.multi = -1; else s.multi = 1
                    }
                    continue
                }
                rvoice = sy.voices[v].range;
    ...
    function set_rest_offset() {
        var s, s2, v, end_time, not_alone, v_s, y, ymax, ymin, shift, dots, dx, v_s_tb = [], sy = cur_sy;
        for (s = tsfirst; s; s = s.ts_next) {
            if (s.invisible)continue;
    ...
    function new_sym(type, p_voice, last_s) {
        s = {type: type, ctx: last_s.ctx, v: p_voice.v, st: p_voice.st, time: last_s.time, next: p_voice.last_sym.next};
        if (s.next)s.next.prev = s;
    ...
    function init_music_line() {
        var p_voice, s, last_s, v, st, nv = voice_tb.length;
        for (v = 0; v < nv; v++) {
            p_voice = voice_tb[v];
        ...
        if (insert_meter & 1) {
            for (v = 0; v < nv; v++) {
                p_voice = voice_tb[v];
                if (cur_sy.voices[v].range < 0 || cur_sy.voices[v].second || cur_sy.staves[cur_sy.voices[v].st].empty || p_voice.meter.a_meter.length == 0)continue;
        ...
        for (v = 0; v < nv; v++) {
            ...
            if (p_voice.bar_start.rbstart)s.rbstart = p_voice.bar_start.rbstart;
            delete p_voice.bar_start
        }
        set_pitch(last_s);
        s = last_s;
        if (s) {
            for (; s; s = s.ts_next)if (s.seqst)break
        }
        if (s) {
    ...
    function set_words(p_voice) {
        var s, pitch, start_flag, nflags, lastnote;
        for (s = p_voice.sym; s; s = s.next) {
            if (s.type == NOTE) {
                pitch = s.notes[0].pit;
                break
    ...
    function set_rb(p_voice) {
        var s2, mx, n, s = p_voice.sym;
        while (s) {
            if (s.type != BAR || !s.rbstart) {
                s = s.next;
                continue
            }
            mx = cfmt.rbmax;
    ...
    function set_global() {
        ...
        for (v = 0; v < nv; v++) {
            p_voice = voice_tb[v];
            set_words(p_voice);
            if (!p_voice.second && !p_voice.norepbra)set_rb(p_voice)
        }
        set_float();
        set_clefs();
        set_pitch(null)
    ...
    function set_indent() {
        var st, v, w, p_voice, p, i, j, font, nv = voice_tb.length, maxw = 0;
        for (v = 0; v < nv; v++) {
            p_voice = voice_tb[v];
            if (cur_sy.voices[v].range < 0)continue;
            st = cur_sy.voices[v].st;
            ...
            while (1) {
                j = p.indexOf("\\n", i);
                if (j < 0)w = strw(p.slice(i)); else w = strw(p.slice(i, j));
                if (w > maxw)maxw = w;
                if (j < 0)break;
                i = j + 1
            }
        ...
        for (s = sym; s; s = s.next) {
            ...
            if (!s.stem && (s.stem = s.multi) == 0) {
                if (beam) {
                    ...
                    if (avg < n)laststem = 1; else if (avg > n || cfmt.bstemdown)laststem = -1;
                    beam = true;
                    s.stem = laststem
                } else {
                    s.stem = s.yav >= 12 ? -1 : 1;
    ...
    function same_head(s1, s2) {
        ...
        if (l1 != l2) {
            ...
            if (l1 < BASE_LEN / 2) {
                if (s2.dots > 0)head = 2; else if (s1.dots > 0)head = 1
        ...
        if (head == 1) {
            s2.nohdi1 = i21;
            s2.nohdi2 = i22;
            for (i2 = i21; i2 < i22; i2++)delete s2.notes[i2].acc;
            for (i2 = 0; i2 <= s2.nhd; i2++)s2.notes[i2].shhd += sh1
        } else {
            s1.nohdi1 = i11;
            s1.nohdi2 = i12;
            for (i1 = i11; i1 < i12; i1++)delete s1.notes[i1].acc;
            for (i1 = 0; i1 <= s1.nhd; i1++)s1.notes[i1].shhd += sh2
        }
        return true
    }

    const w_note = {full: 3.5, empty: 3.7, oval: 5, square: 7};
    ...
    function set_left(s, left) {
        ...
        for (m = 0; m <= s.nhd; m++) {
            ...
            i = s.notes[m].pit * 2;
            if (i < 0)i = 0; else if (i >= MAXPIT - 1)i = MAXPIT - 2;
    ...
    function set_right(s, right) {
        ...
        if (flags) {
            if (s.stem > 0) {
                if (s.xmx == 0)i = s.notes[s.nhd].pit * 2; else i = s.notes[0].pit * 2;
                i += 4;
                if (i < 0)i = 0;
    ...
    function set_overlap() {
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            ...
            if (!s2)continue;
            s1 = s;
            if (cur_sy.voices[s1.v].range < cur_sy.voices[s2.v].range)s2.dot_low = true; else s1.dot_low = true;
            ...
            if (s1.dots) {
                ...
                if (!sd) {
                    ...
                    for (i1 = 0; i1 <= s1.nhd; i1++) {
                        i = s1.notes[i1].pit;
                        if (!(i & 1)) {
                            if (!s1.dot_low)i++; else i--
                        }
                        i *= 2;
                        if (i < 1)i = 1; else if (i >= MAXPIT - 1)i = MAXPIT - 2;
                        if (pl[i] > d2)d2 = pl[i];
        ...
        for (s = tsfirst; s; s = s.ts_next) {
            if (s.type != NOTE) {
                ...
                for (g = s.extra; g; g = g.next) {
                    ...
                    ymx += 2;
                    ymn -= 2;
                    if (ymn < ymin)ymin = ymn; else if (ymx > ymax)ymax = ymx;
                    g.ymx = ymx;
                    g.ymn = ymn
                }
                s.ymx = ymax;
    ...
    function check_bar(s) {
        ...
        if (!p_voice.bar_start)p_voice.bar_start = clone(s);
        if (bar_type[0] != ":") {
            p_voice.bar_start.bar_type = bar_type;
            if (s.prev && s.prev.type == BAR)unlksym(s); else s.bar_type = "|";
            return
        }
        if (bar_type == "::") {
            p_voice.bar_start.bar_type = "|:";
    ...
    function set_piece() {
        ...
        function reset_staff(st) {
            var p_staff = staff_tb[st], sy_staff = sy.staves[st];
        ...
        function set_top_bot() {
            ...
            for (st = 0; st <= nstaff; st++) {
                ...
                for (i = 0; i < l - 1; i++)if (p_staff.stafflines[i] != ".")break;
        ...
        nv = voice_tb.length;
        for (v = 0; v < nv; v++) {
            if (voice_tb[v].sym && voice_tb[v].sym.time <= tsnext.time) {
    ...
    function set_sym_glue(width) {
        var space, beta0, alfa, beta, min, g, spafac;
        ...
        if (x >= width) {
            ...
            alfa = 0;
            if (xmax > x)beta = (width - x) / (xmax - x); else beta = 1;
            if (beta > beta0) {
        ...
        if (some_grace) {
            for (s = tsfirst; s; s = s.ts_next) {
                ...
                for (g = s.extra; g; g = g.next)if (g.type == NOTE)g.x += x
            }
        }
    }
    ...
    function output_music() {
        ...
        indent = set_indent();
        if (cfmt.singleline) {
            lwidth = get_ck_width() + get_width(tsfirst, null, indent);
            cfmt.pagewidth = (lwidth + cfmt.leftmargin + cfmt.rightmargin) * cfmt.scale
        } else {
    ...
    function new_block(subtype) {
        if (parse.state == 2)goto_tune();
        var s = {type: BLOCK, subtype: subtype};
        curvoice = voice_tb[0];
        if (curvoice.last_sym)curvoice.last_sym.eoln = true;
        sym_link(s);
        return s
    }
    ...
    function do_pscom(text) {
        ...
        switch (cmd) {
            ...
            case"repeat":
                ...
                s = {type: FORMAT, fmt_type: "repeat", repeat_k: k};
                if (curvoice.last_sym.type == BAR)s.repeat_n = n; else s.repeat_n = -n;
                sym_link(s);
                return;
            case"sep":
            ...
            case"setbarnb":
                ...
                if (parse.state >= 2)glovar.new_nbar = val; else cfmt.measurefirst = val;
                return;
            case"staff":
                if (parse.state != 3) {
                    if (parse.state != 2)return;
                ...
                var st;
                if (param[0] == "+" || param[0] == "-")st = curvoice.cst + val; else st = val - 1;
                if (st < 0 || st > nstaff) {
            ...
            case"transpose":
                ...
                if (!curvoice) {
                    if (parse.state == 0)cfmt.transpose = val; else cfmt.transpose += val;
                    return
                }
            ...
            case"voicecolor":
                if (parse.state < 2)return;
                if (parse.state == 2)goto_tune();
                s = {type: FORMAT, fmt_type: "voicecolor"};
                ...
                default:
                    switch (item.slice(0, 4)) {
                        ...
                        case"perc":
                            ...
                            break;
                        default:
                            if ("GFC".indexOf(item[0]) >= 0)s = new_clef(item); else if (item[item.length - 1] == "=")a.shift();
                            break
                    }
                    break
            }
        ...
        if (!key_end) {
            ...
            if (c == "^" || c == "_" || c == "=") {
                ...
                do {
                    ...
                    var acc = {pit: note.pit, acc: note.acc};
                    s.k_a_acc.push(acc);
                    c = tmp.char();
    ...
    function new_meter(text) {
        ...
        if (parse.state != 3) {
            ...
            if (parse.state >= 1) {
                if (!glovar.ulen) {
                    if (wmeasure <= 1 || wmeasure >= BASE_LEN * 3 / 4)glovar.ulen = BASE_LEN / 8; else glovar.ulen = BASE_LEN / 16
                }
                for (v = 0; v < voice_tb.length; v++) {
                    voice_tb[v].meter = s;
    ...
    function new_tempo(text) {
        ...
        var i = 0, j, c, nd, tmp, s = {type: TEMPO, ctx: parse.ctx, istart: parse.istrt, iend: parse.iend};
        if (text[0] == '"') {
            i = text.indexOf('"', 1);
            if (i < 0) {
    ...
    function do_info(info_type, text) {
        switch (info_type) {
            ...
            case"L":
                ...
                if (parse.state == 2)goto_tune();
                if (parse.state != 3)glovar.ulen = Number(text); else curvoice.ulen = Number(text);
                break;
            case"M":
                new_meter(text);
                break;
            ...
            case"N":
            case"R":
                if (!info[info_type])info[info_type] = text; else info[info_type] += "\n" + text;
                break;
            default:
    ...
    function adjust_dur(s) {
        ...
        if (time == 0) {
            ...
            if (s2 && s2.type == REST && s2.invisible) {
                ...
                if (s2.prev)s2.prev.next = s2.next; else curvoice.sym = s2.next;
        ...
        for (; s2; s2 = s2.next) {
            ...
            s2.dots = res[1];
            s2.nflags = res[2];
            if (s2.nflags <= -2)s2.stemless = true; else delete s2.stemless
        }
        curvoice.time = s.time = time
    }

    function new_bar() {
        var line = parse.line, s = {type: BAR, ctx: parse.ctx, multi: 0}, s2, c, bar_type;
        ...
        switch (bar_type[bar_type.length - 1]) {
            case"[":
                if (!s.rbstart) {
        ...
        if (bar_type[0] == "]") {
            s.rbstop = 2;
            if (bar_type.length != 1)bar_type = bar_type.slice(1); else s.invisible = true
        }
        ...
        while (i < p.length) {
            switch (p[i]) {
                ...
                case"[":
                    ...
                    if (brace + bracket == 0)flags |= OPEN_BRACKET; else flags |= OPEN_BRACKET2;
                    bracket++;
                    flags_st <<= 8;
                ...
                case"{":
                    ...
                    if (bracket == 0)flags |= OPEN_BRACE; else flags |= OPEN_BRACE2;
                    brace++;
                    flags_st <<= 8;
                    flags_st |= OPEN_BRACE;
                    break;
                ...
                default:
                    ...
                    p_flags = {v: new_voice(voice_id).v};
                    for (; i < p.length; i++) {
                        switch (p[i]) {
                            case" ":
                            case"	":
                            ...
                            case"]":
                                ...
                                bracket--;
                                if (brace + bracket == 0)flags |= CLOSE_BRACKET; else flags |= CLOSE_BRACKET2;
                                flags_st >>= 8;
                                continue;
                            case"}":
                                ...
                                brace--;
                                if (bracket == 0)flags |= CLOSE_BRACE; else flags |= CLOSE_BRACE2;
                                flags &= ~FL_VOICE;
                                flags_st >>= 8;
                                continue;
                            case")":
    ...
    function parse_voice(param) {
        ...
        if (curvoice.time == undefined) {
            ...
            if (parse.state == 3 && staves_found < 0) {
                ...
                par_sy.staves[nstaff] = {stafflines: "|||||", staffscale: 1}
            }
        }
    ...
    function parse_acc_pit(line) {
        ...
        if (acc && (c >= "1" && c <= "9") || c == "/") {
            ...
            micro_n = nd[0];
            micro_d = nd[1];
            if (micro_d == 1)micro_d = curvoice.microscale; else micro_d *= 2;
            c = line.char()
        }
        ...
        note = {pit: pit, apit: pit, shhd: 0, shac: 0, ti1: 0};
        if (acc) {
            note.acc = acc;
            if (micro_n) {
    ...
    function set_map(note) {
        ...
        if (note.acc)an = ["__", "_", "", "^", "^^", "="][note.acc + 2]; else an = "";
        nn = an + bn;
    ...
    function new_note(grace, tuplet_fact) {
        ...
        s = {type: NOTE, ctx: parse.ctx, stem: 0, multi: 0, nhd: 0, xmx: 0};
        s.istart = parse.bol + line.index;
        if (grace) {
            s.grace = true
        } else {
        ...
        switch (c) {
            ...
            case"Z":
                s.type = MREST;
                c = line.next_char();
                if (c > "0" && c <= "9")s.nmes = line.get_int(); else s.nmes = 1;
            ...
            case"y":
                ...
                s.invisible = true;
                c = line.next_char();
                if (c >= "0" && c <= "9")s.width = line.get_int(); else s.width = 10;
                break;
            case"x":
                s.invisible = true;
            case"z":
            ...
            default:
                ...
                while (1) {
                    ...
                    while (1) {
                        switch (c) {
                            case")":
                                if (note.sl2)note.sl2++; else note.sl2 = 1;
                                if (s.sl2)s.sl2++; else s.sl2 = 1;
                                c = line.next_char();
                                continue;
                            case"-":
                                note.ti1 = parse_vpos();
        ...
        if (s.notes) {
            if (s.type == NOTE && curvoice.transpose)note_transp(s);
            if (!s.grace) {
            ...
            if (s.type == NOTE) {
                ...
                if (dur == curvoice.wmeasure) {
                    if (dur < BASE_LEN * 2)dur = BASE_LEN; else if (dur < BASE_LEN * 4)dur = BASE_LEN * 2; else dur = BASE_LEN * 4
                }
        ...
        while (1) {
            ...
            switch (type[0]) {
                ...
                case"(":
                    c = line.next_char();
                    if (c > "0" && c <= "9") {
                        var pplet = line.get_int(), qplet = [0, 1, 3, 2, 3, 0, 2, 0, 3, 0][pplet], rplet = pplet, c = line.char();
                        if (c == ":") {
                            c = line.next_char();
                            if (c > "0" && c <= "9") {
                                qplet = line.get_int();
                ...
                case")":
                    ...
                    if (s.slur_end)s.slur_end++; else s.slur_end = 1;
                    break;
                case"!":
                    if (!a_dcn)a_dcn = [];
                    if (type.length > 1) {
                ...
                case"-":
                    ...
                    for (i = 0; i <= s.nhd; i++) {
                        if (!s.notes[i].ti1)s.notes[i].ti1 = tie_pos; else if (s.nhd == 0)line.error("Too many ties")
                    }
                    s.ti1 = true;
                    continue;
                case"[":
    ...
    function out_str(str) {
        var i, c, n_font, o_font = gene.curfont, c_font = o_font;
        for (i = 0; i < str.length; i++) {
            ...
            switch (c) {
                ...
                case"$":
                    ...
                    if (n_font == c_font)continue;
                    if (c_font != o_font)output.push("</tspan>");
                    c_font = n_font;
                    if (c_font == o_font)continue;
                    c = '<tspan\n	class="f' + n_font.fid + cfmt.fullsvg + '">';
                    break
            }
            output.push(c)
        }
    ...
    function xy_str(x, y, str, action, dx) {
        ...
        out_sxsy(x, '" y="', y);
        if (action == "c")output.push('" text-anchor="middle">'); else if (action == "j")output.push('" textLength="' + dx.toFixed(2) + '">'); else if (action == "r")output.push('" text-anchor="end">'); else output.push('">');
        out_str(str);
        output.push("</text>\n")
    }
    ...
    function write_title(title, is_subtitle) {
        ...
        if (cfmt.titleleft)xy_str(0, sz * .2, title); else xy_str((cfmt.pagewidth - cfmt.leftmargin - cfmt.rightmargin) / 2 / cfmt.scale, sz * .2, title, "c")
    }
    ...
    function put_inf2r(x, y, str1, str2, action) {
        ...
        if (!str2)xy_str(x, y, str1, action); else xy_str(x, y, str1 + " (" + str2 + ")", action)
    }
    ...
    function write_text(text, action) {
        ...
        switch (action) {
            ...
            case"j":
                ...
                while (1) {
                    i = text.indexOf("\n\n", j);
                    if (i < 0)words = text.slice(j); else words = text.slice(j, i);
                    words = words.split(/\s+/);
                    var w = 0, k = 0;
    ...
    function write_headform(lwidth) {
        ...
        if (fmt[fmt.length - 1] == "+")fmt = fmt.slice(0, -1) + "l";
        fmt += "\n";
        var ya = {l: cfmt.titlespace, c: cfmt.titlespace, r: cfmt.titlespace}, xa = {
            l: 0,
            c: lwidth * .5,
            r: lwidth
        }, yb = {}, str;
        ...
        while (1) {
            ...
            while (1) {
                ...
                if (c == "\n")break;
                align = p[j++];
                if (align == "+")align = p[j + 1]; else if (yb[align] != 0)continue;
                str = info_val[c];
                if (!str)continue;
            ...
            while (1) {
                ...
                if (align == "+") {
                    ...
                    if (info_val[c].length > 0) {
                        if (str)str += " " + info_val[c].shift(); else str = " " + info_val[c].shift()
                    }
                }
                font_name = info_font[c];
                if (!font_name)font_name = "history";
    ...
    function write_heading() {
        var i, j, area, composer, origin, rhythm, down1, down2, lwidth = (cfmt.pagewidth - cfmt.leftmargin - cfmt.rightmargin) / cfmt.scale;
    ...
    function set_g() {
        ...
        if (stv_g.scale != 1) {
            if (stv_g.st >= 0) {
                if (staff_tb[stv_g.st].y == 0)output.push("!S" + stv_g.st + "!"); else output.push(staff_tb[stv_g.st].scale_str)
            } else {
                if (staff_tb[0].y == 0)output.push("!V" + stv_g.v + "!"); else output.push(voice_tb[stv_g.v].scale_str)
            }
        }
        if (stv_g.color) {
    ...
    function set_sscale(st) {
        ...
        if (st >= 0)new_scale = staff_tb[st].staffscale; else new_scale = 1;
        if (st >= 0 && new_scale != 1)dy = staff_tb[st].y; else dy = posy;
    ...
    function anno_start(s) {
        ...
        var type = s.type, h = s.ymx - s.ymn + 4;
    ...
    function out_sxsy(x, sep, y) {
        if (!stv_g.trans) {
            ...
            if (stv_g.scale != 1) {
                if (stv_g.st < 0)y = (posy - y) / stv_g.scale; else y = stv_g.dy - y
            } else {
                y = posy - y
            }
        }
    ...
    function xypath(x, y, fill) {
        if (fill)output.push('<path class="fill" d="m'); else output.push('<path class="stroke" d="m');
        out_sxsy(x, " ", y);
        output.push("\n")
    }
    ...
    function out_bar(x, y, h) {
        x += posx;
        y = posy - y;
        output.push('<path class="stroke" stroke-width="1" d="m' + x.toFixed(2) + " " + y.toFixed(2) + "v" + (-h).toFixed(2) + '"/>\n')
    }
    ...
    function out_hyph(x, y, w) {
        ...
        if (w > 15)n = Math.floor((w - 15) / d); else n = 0;
        x += (w - d * n - 5) / 2;
        output.push('<path class="stroke" stroke-width="1.2"\n	stroke-dasharray="5,');
    ...
    function out_thbar(x, y, h) {
        x += posx + 1.5;
        y = posy - y;
        output.push('<path class="stroke" stroke-width="3"\n	d="m' + x.toFixed(2) + " " + y.toFixed(2) + "v" + (-h).toFixed(2) + '"/>\n')
    }
    ...
    const deco_str_style = {
        crdc: {dx: 0, dy: 5, style: 'style="font:italic 14px serif"'},
        dacs: {dx: 0, dy: 3, style: 'style="font:16px serif" text-anchor="middle"'},
        fng: {dx: -3, dy: 1, style: 'style="font:8px Bookman"'},
        pf: {dx: 0, dy: 5, style: 'style="font:bold italic 16px serif"'}
    };

    function out_deco_str(x, y, name, str) {
        var a, f, a_deco = deco_str_style[name];
        if (!a_deco) {
            a_deco = {dx: 0, dy: 0, style: 'style="font:14px serif"'}
        }
        x += a_deco.dx;
        y += a_deco.dy;
    ...
    function out_cresc(x, y, val, defl) {
        ...
        if (defl.nost) {
            out_sxsy(x, " ", y + 3.2);
            output.push("l" + val.toFixed(2) + " -2.2m0 8l" + (-val).toFixed(2) + ' -2.2"/>\n')
        } else {
            out_sxsy(x, " ", y + 5);
            output.push("l" + val.toFixed(2) + " -4m0 8l" + (-val).toFixed(2) + ' -4"/>\n')
        }
    }
    ...
    var deco_val_tb = {arp: out_arp, cresc: out_cresc, dim: out_dim, ltr: out_ltr};

    function out_deco_val(x, y, name, val, defl) {
        if (deco_val_tb[name])deco_val_tb[name](x, y, val, defl); else error(1, null, "No function for decoration '" + name + "'")
    }
    ...
    function svg_flush() {
        ...
        if (cfmt.scale == 1)user.img_out('<g stroke-width=".7">'); else user.img_out('<g stroke-width=".7"> transform="scale(' + cfmt.scale.toFixed(2) + ')">');
        if (svgobj) {
            svgobj.setg(0);
            output.push(svgbuf);
            svgbuf = ""
    ...
    function sym_link(s) {
        if (!curvoice.ignore) {
            ...
            if (curvoice.last_sym)curvoice.last_sym.next = s; else curvoice.sym = s;
            curvoice.last_sym = s
        }
        s.v = curvoice.v;
        s.st = curvoice.cst;
    ...
    function sort_all() {
        var s, s2, p_voice, v, time, w, wmin, ir, multi, prev, new_sy, fl, nb, nv = voice_tb.length, vtb = [], vn = [], mrest_time = -1;
        for (v = 0; v < nv; v++)vtb.push(voice_tb[v].sym);
        var sy = cur_sy, set_sy = true;
        while (1) {
            if (set_sy) {
                set_sy = false;
                fl = 1;
            ...
            for (ir = 0; ir < vn.length; ir++) {
                ...
                if (s.type == MREST) {
                    if (s.nmes == 1)mrest_expand(s); else if (multi > 0)mrest_time = time
                }
            }
            if (wmin > 127)break;
            if (time == mrest_time) {
            ...
            for (ir = 0; ir < vn.length; ir++) {
                ...
                if (s.type == STAVES) {
                    sy = sy.next;
                    set_sy = new_sy = true;
                    if (s.prev)s.prev.next = s.next; else voice_tb[v].sym = s.next;
        ...
        for (v = 0; v < voice_tb.length; v++) {
            ...
            for (; s; s = s.next) {
                switch (s.type) {
                    ...
                    case MREST:
                        if (!ns)continue;
                        s2 = {type: SPACE, width: -1, invisible: true, v: s.v, st: s.st, time: s.time};
                        s2.next = s;
                        s2.prev = s.prev;
                        s2.prev.next = s2;
                        s.prev = s2;
                ...
                if (s.grace) {
                    ...
                    delete s2.notes;
                    s2.next = s.next;
                    if (s2.next)s2.next.prev = s2; else p_voice.last_sym = s2;
                    s2.prev = s;
                    s.next = s2;
                    s = s2
                }
                ...
                delete s.prev.next;
                s.prev = ns.prev;
                if (s.prev)s.prev.next = s; else p_voice.sym = s;
                delete ns.prev;
                ns = null
            }
            if (ns) {
    ...
    function dupl_voice() {
        ...
        for (v = 0; v < voice_tb.length; v++) {
            ...
            for (; s; s = s.next) {
                ...
                sym_link(s2);
                if (p_voice2.second)s2.second = true; else delete s2.second;
                if (p_voice2.floating)s2.floating = true; else delete s2.floating;
                delete s2.a_ly;
                g = s2.extra;
                if (!g)continue;
                g2 = clone(g);
    ...
    function new_system() {
        var st, v, sy_new = {voices: [], staves: [], top_voice: 0};
        if (!par_sy) {
    ...
    function set_bar_num() {
        ...
        var bar_time = s.time + wmeasure, bar_num = gene.nbar;
        for (; s; s = s.ts_next) {
            switch (s.type) {
                case CLEF:
                    if (s.new_sy)break;
                ...
                case BAR:
                    ...
                    do {
                        if (s2.type == BAR && s2.text && !cfmt.contbarnb) {
                            if (s2.text[0] == "1")bar_rep = bar_num; else bar_num = bar_rep;
                            break
                        }
                        s2 = s2.next
    ...
    function key_transp(s_key, transpose) {
        var sf, t;
        if (transpose > 0)t = Math.floor(transpose / 3); else t = -Math.floor(-transpose / 3);
    ...
    function note_transp(s) {
        var i, j, n, d, a, acc, i1, i3, i4, note, m = s.nhd, sf_old = curvoice.okey.k_sf, i2 = curvoice.ckey.k_sf - sf_old, dp = cgd2cde[(i2 + 4 * 7) % 7], t = curvoice.transpose;
        if (t > 0) {
        ...
        for (i = 0; i <= m; i++) {
            ...
            if (note.acc) {
                ...
                if (a == 3 || acc_same_pitch(note.pit))continue
    ...
    function get_staves(cmd, param) {
        ...
        for (i = 0; i < a_flags.length; i++) {
            ...
            if (flags & FL_VOICE) {
                ...
                st++;
                if (!par_sy.staves[st]) {
                    par_sy.staves[st] = {stafflines: "|||||", staffscale: 1}
                }
    ...
    function get_vover(type) {
        ...
        if (!p_voice2) {
            ...
            if (f_clone) {
                ...
                v3 = p_voice3.v;
                par_sy.voices[v3] = {second: true, scale: curvoice.clone.scale, range: range + 2};
        ...
        if (!vover) {
            vover = {bar: true, mxtime: curvoice.time, p_voice: curvoice};
    ...
    function get_key_clef(kc) {
        ...
        if (parse.state == 1) {
            ...
            if (cfmt.transpose)key_transp(s_key, cfmt.transpose);
            for (v = 0; v < voice_tb.length; v++) {
                p_voice = voice_tb[v];
                p_voice.ckey = s_key;
        ...
        if (curvoice.transpose)key_transp(s_key, curvoice.transpose);
    ...
    function new_voice(id) {
        ...
        if (cfmt.voicemap)p_voice.map = cfmt.voicemap;
    ...
    function get_voice(text) {
        var s_clef = parse_voice(text), v = curvoice.v;
        if (par_sy.voices[v].range < 0) {
            if (cfmt.alignbars) {
    ...
    function goto_tune() {
        ...
        for (v = 0; v < voice_tb.length; v++) {
            ...
            if (p_voice.key.k_bagpipe && p_voice.pos.ste == 0)p_voice.pos.ste = SL_BELOW;
        ...
        if (staves_found < 0) {
            ...
            for (v = 0; v <= nstaff; v++) {
                ...
                par_sy.voices[v].range = v;
                par_sy.staves[v] = {stafflines: "|||||", staffscale: 1}
            }
            par_sy.nstaff = nstaff
        }
        parse.state = 3;
    ...
    function get_lyrics(text, cont) {
        var s, word, p, i, ly;
        if (curvoice.ignore)return;
        if (curvoice.pos.voc != SL_HIDDEN)curvoice.have_ly = true;
        if (cont) {
        ...
        while (1) {
            ...
            if (word && s.pos.voc != SL_HIDDEN) {
                if (word.match(/^\$\d/)) {
                    if (word[1] == "0")set_font("vocal"); else set_font("u" + word[1]);
                    word = word.slice(2)
                }
                ly = {t: word, font: gene.curfont, w: strw(word)};
                if (!s.a_ly)s.a_ly = [];
        ...
        for (i = 0; i < a_ly.length; i++) {
            ...
            if (p[0] >= "0" && p[0] <= "9" && p.length > 2 || p[1] == ":" || p[0] == "(" || p[0] == ")") {
                if (p[0] == "(") {
                    ...
                    if (j > 0)sz = strw(p.slice(0, j)); else sz = w
                }
            ...
            for (k = s.next; k; k = k.next) {
                switch (k.type) {
                    case NOTE:
                    case REST:
                        if (!k.a_ly || !k.a_ly[i] || k.a_ly[i].w == 0)xx -= 9; else if (k.a_ly[i].t == "-\n" || k.a_ly[i].t == "_\n")xx -= shift; else break;
                        if (xx <= 0)break;
                        continue;
                    case CLEF:
                    case METER:
    ...
    function draw_lyrics(p_voice, nly, y, incr) {
        ...
        for (; j != nly; j += incr) {
            ...
            for (s = p_voice.sym; ; s = s.next)if (s.type != CLEF && s.type != KEY && s.type != METER)break;
            if (s.prev)lastx = s.prev.x; else tsfirst.x;
            x0 = 0;
            lskip = font.size * 1.1;
            for (; s; s = s.next) {
                if (s.a_ly)ly = s.a_ly[j]; else ly = null;
                if (!ly) {
                    switch (s.type) {
                        case REST:
                        case MREST:
                ...
                if (p == "-\n" || p == "_\n") {
                    ...
                    if (p[0] == "-")hyflag = true; else lflag = true;
                    x0 = s.x - shift;
                    continue
                }
                x0 = s.x - shift;
            ...
            if (incr > 0)y -= lskip; else y += lskip
        }
        if (incr > 0)y += lskip;
        return y
    }
    ...
    function draw_all_lyrics() {
        ...
        for (v = 0; v < voice_tb.length; v++) {
            ...
            nly_tb[v] = nly;
            if (nly == 0)continue;
            if (p_voice.pos.voc != 0)above_tb[v] = p_voice.pos.voc == SL_ABOVE; else if (voice_tb[p_voice.v + 1] && voice_tb[p_voice.v + 1].st == st && voice_tb[p_voice.v + 1].have_ly)above_tb[v] = true; else above_tb[v] = false;
            if (above_tb[v])lyst_tb[st].a = true; else lyst_tb[st].b = true
        }
        i = 0;
        for (v = 0; v < voice_tb.length; v++) {
            p_voice = voice_tb[v];
    ...
    function parse_gchord(type) {
        ...
        if (!gchord)gchord = cnv_escape(text); else gchord += "\n" + cnv_escape(text)
    }
    ...
    function gch_tr1(p) {
        var new_txt, l, latin, n, i1, i2, i3, i4, ix, a, ip, ip2;
        latin = 0;
        switch (p[0]) {
            case"A":
            case"B":
        ...
        i4 = cgd2cde[(i3 + 16 * 7) % 7];
        i1 = (Math.floor((i3 + 1 + 21) / 7) + 2 - 3 + 32 * 5) % 5;
        if (!latin)new_txt = note_names[i4] + acc_name[i1]; else new_txt = latin_names[i4] + acc_name[i1];
        ip2 = p.indexOf("/", ip);
        ...
        i4 = cgd2cde[(i3 + 16 * 7) % 7];
        i1 = (Math.floor((i3 + 1 + 21) / 7) + 2 - 3 + 32 * 5) % 5;
        return note_names[i4] + acc_name[i1] + p.slice(ip2)
    }
    ...
    function gch_transp(s) {
        ...
        gch.text = gch_tr1(p, 0)
    }

    function gch_build(s) {
        if (curvoice.pos.gch == SL_HIDDEN)return;
        var src, gch, w, xspc, x_abs, y_abs, ix, n, pos = curvoice.pos.gch == SL_BELOW ? -1 : 1, gch_font = get_font("gchord"), ann_font = get_font("annotation"), h_gch = gch_font.size, h_ann = ann_font.size, y_above = 0, y_below = 0, y_left = 0, y_right = 0, box = cfmt.gchordbox, sep = "\n";
        s.a_gch = [];
        src = new scanBuf;
        src.buffer = gchord;
        src.index = 0;
        ...
        if (curvoice.transpose)gch_transp(s);
        const GCHPRE = .4;
        n = s.a_gch.length;
        for (ix = 0; ix < n; ix++) {
            gch = s.a_gch[ix];
    ...
    function draw_gchord(s, gchy_min, gchy_max) {
        ...
        for (ix = 0; ix < n; ix++) {
            ...
            switch (gch.type) {
                ...
                case"@":
                    y = gch.y + yav;
                    if (y > 0)y_set(s.st, 1, x, 1, y + h * .8 + 3); else y_set(s.st, 0, x, 1, y - h * .2);
                    break
            }
